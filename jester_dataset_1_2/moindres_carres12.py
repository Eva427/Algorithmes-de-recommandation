...
def Alternating_Least_Square(R_train, param_reg, nf, it_max) : #R user-item matrix 
    ...
    return U, M, cond0, mae


#U,M, rmse, mae = Alternating_Least_Square(R_train, 0.07, 10, 20)
#lambda = 0.6 => RMSE = 4.186952112239503
#lambda = 1 => RMSE = 4.44618824113833
#lambda = 0.4 => RMSE = 4.09409847735052
#lambda = 0.1 => RMSE =  4.161376736522084
#lambda = 0.07 => RMSE = 

U,M, rmse, mae = Alternating_Least_Square(R_train, 0.36, 20, 20)
...
for i in x : 
    ...
    U,M, rmse, mae = Alternating_Least_Square(R_train, 0.2, i, 20)
    #Resultat = np.dot(np.transpose(U),M)
    stock_rmse2.append(rmse)
    stock_mae2.append(mae)
...
for i in x : 
    ...
    U,M, rmse, mae = Alternating_Least_Square(R_train, 0.3, i, 20)
    #Resultat = np.dot(np.transpose(U),M)
...
y5=np.array(stock_rmse5)
#plt.scatter(x,y)
#plt.plot(x,y2, "x-", c="#1E788F", label = 'lambda = 0.4')
