...
R_predicted, training_process = Funk_SVD.compute_Funk_SVD(R_train, K=15, alpha=0.00002, beta=0.2, nb_iterations=10)

"""
TESTS
K=15, alpha=0.0035,  beta=0.02, nb_iterations=10 -> nan 
K=15, alpha=0.0002, beta=0.02, nb_iterations=10 -> ok mais valeurs très éloignées de la réalité
K=100, alpha=0.0002, beta=0.02, nb_iterations=10 -> ok mais valeurs très éloignées de la réalité
K=15, alpha=0.0002, beta=0.2, nb_iterations=10 -> ok mais valeurs très éloignées de la réalité
K=15, alpha=0.00002, beta=0.2, nb_iterations=10 -> ok mais valeurs très éloignées de la réalité
"""

# comme le temps de calcul est très long, on sauve R_predicted et traning_process dans un fichier excel
df5 = pd.DataFrame(R_predicted)
df6 = pd.DataFrame(training_process)
df5.to_excel('R_predicted.xlsx', index = False, header=False)
df6.to_excel('training_process.xlsx', index = False, header=False) 
